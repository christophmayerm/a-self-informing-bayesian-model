...
requires(SIZE > 0) struct Hypercube {
...
template <typename T, int SIZE>
requires(SIZE >
         0) constexpr void merge_two_hypercubes(Hypercube<T, SIZE>& hc_in_out,
...
template <typename T, int SIZE>
requires(SIZE > 0) [[nodiscard]] constexpr auto merge_hypercubes(
    const std::vector<Hypercube<T, SIZE>>& single_hcs) noexcept -> std::vector<Hypercube<T, SIZE>> {
